...
function sendWA(to, msg) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({ messaging_product:'whatsapp', to: to.replace(/\D/g,''), type:'text', text:{body:msg} });
    const opts = {
      hostname:'graph.facebook.com', path:`/v19.0/${CONFIG.PHONE_NUMBER_ID}/messages`,
      method:'POST', headers:{'Authorization':`Bearer ${CONFIG.ACCESS_TOKEN}`,'Content-Type':'application/json','Content-Length':Buffer.byteLength(body)}
    };
...
const server = http.createServer(async (req, res) => {
  ...
  if(req.method==='GET' && req.url.startsWith('/webhook')){
    ...
    if(u.searchParams.get('hub.verify_token')===CONFIG.VERIFY_TOKEN){res.writeHead(200);res.end(u.searchParams.get('hub.challenge'));return;}
...
server.listen(3000,()=>console.log('Backend Pastarrino OK :3000'));
